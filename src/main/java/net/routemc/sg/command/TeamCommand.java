    ...
    public boolean onCommand(CommandSender commandSender, Command command, String label, String[] args) {
        if (commandSender instanceof Player) {
            ...
            if (RouteSG.getGameTask().getState() == GameState.Lobby) {
                ...
                TeamManager m = RouteSG.getInstance().getTeamManager();
                if (args.length == 1) {
                    ...
                    if (args[0].equalsIgnoreCase("invite")) {
                        if (gamePlayer.isInTeam()) {
                            for (int i = 1; i < args.length; i++) {
                                ...
                                } else if (RouteSG.getGameTask().getGamePlayer(Bukkit.getPlayer(targetName)) == null) {
                                    player.sendMessage(ChatColor.RED + targetName + " is offline.");
                                } else {
                                    gamePlayer.getInTeam().invite(gamePlayer, RouteSG.getGameTask().getGamePlayer(Bukkit.getPlayer(targetName)));
                                }
                            }
                        } else {
