    ...
    public boolean onCommand(CommandSender commandSender, Command command, String label, String[] args) {
        if (commandSender instanceof Player) {
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                else if(RouteSG.getGameTask().getGamePlayer(target).getInTeam() != null){
                    player.sendMessage(Style.RED + "Error: " + name + " is in team.");
                }
                else if(RouteSG.getInstance().getObserverPlayers().contains(name.toLowerCase())){
                    player.sendMessage(Style.RED + "Error: " + name + " has already been observer.");
                }else{
                    BukkitReflection.setMaxPlayers(RouteSG.getInstance().getServer(), Bukkit.getServer().getMaxPlayers() + 1);
                    RouteSG.getInstance().getObserverPlayers().add(name.toLowerCase());
                    GamePlayer gamePlayer = RouteSG.getGameTask().getGamePlayer(target);
